...
a3 = sigmoid(z3);
t1r = [Theta1(:,2:size(Theta1,2))];
t2r = [Theta2(:,2:size(Theta2,2))];
for i = 1:m
    J = J + (1/m)*sum(-Y(i,:).*log(a3(i,:)) - (1-Y(i,:)).*log(1-a3(i,:)));
end
Theta1sqr = t1r.^2;
Theta2sqr = t2r.^2;
J = J + (0.5*lambda/m)*(sum(Theta1sqr(:)) + sum(Theta2sqr(:)));
...
Theta1_grad = (1/m)*(Theta1_grad + (a1'*delta2)');
Theta2_grad = (1/m)*(Theta2_grad + (a2'*delta3)');
